...
//Spade Update Function===================================================================================================
if(isset($_POST["upd_spade"]))
{
    $target = "images/".basename($_FILES['image']['name']);
    $errors = array();
    $required_fields = array('title', 'summary', 'main_post', 'dt', 'category');
    foreach($required_fields as $fieldname){
        //var_dump($_POST[$fieldname]);
        if(!isset($_POST[$fieldname]) || (empty($_POST[$fieldname])))   {
            $errors[] = $fieldname;
    ...
    if(empty($errors))
    {
        ...
        $image = $_FILES['image']['name'];
        if(empty($image)){
            $query = "UPDATE spade SET title = '{$title}', category = '{$category}', dt = '{$date}', summary = '{$summary}', main_post = '{$main_post}' WHERE id = {$id} ";
        }else{
            $query = "UPDATE spade SET title = '{$title}', category = '{$category}', dt = '{$date}', picture = '{$image}', summary = '{$summary}', main_post = '{$main_post}' WHERE id = {$id} ";
            move_uploaded_file($_FILES['image']['tmp_name'], $target);
        }
        if($result = $mysqli->query($query) or die($mysqli->error))
        {
...
//Poetry Update Function===================================================================================================
if(isset($_POST["upd_poetry"]))
{
    $target = "images/".basename($_FILES['image']['name']);
    $errors = array();
    $required_fields = array('title', 'summary', 'main_post', 'dt', 'author');
    foreach($required_fields as $fieldname){
        //var_dump($_POST[$fieldname]);
        if(!isset($_POST[$fieldname]) || (empty($_POST[$fieldname])))   {
            $errors[] = $fieldname;
    ...
    if(empty($errors))
    {
        ...
        $image = $_FILES['image']['name'];
        if(empty($image)){
            $query = "UPDATE poetry SET title = '{$title}', author = '{$author}', dt = '{$date}', summary = '{$summary}', main_post = '{$main_post}' WHERE id = {$id} ";
        }else{
            $query = "UPDATE poetry SET title = '{$title}', author = '{$author}', dt = '{$date}', picture = '{$image}', summary = '{$summary}', main_post = '{$main_post}' WHERE id = {$id} ";
            move_uploaded_file($_FILES['image']['tmp_name'], $target);
        }
        if($result = $mysqli->query($query) or die($mysqli->error))
        {
            ...
            {
            echo "<script>alert(\"The poetry post update failed\")<script>";
            }
